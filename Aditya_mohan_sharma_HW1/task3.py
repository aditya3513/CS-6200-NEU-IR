...
#get user input
url = input("please enter the url,  if you dont enter anything it will take Carbon_footprint as default as given in assignment : ")

keyword_input = input("enter keyword, if you dont enter anything it will take green as default : ")


#given input to us
if not url:
	origin = "https://en.wikipedia.org/wiki/Carbon_footprint"
else:
	origin = url
...
visited_link_file = open("visited_link_file_q3.txt","w+")
...
def getChildLinks(parent):
	...
	file_name = "html_files_q3/" + file_name + ".txt"
	...
	for a_tag in all_a_tags:
		...
		if(":" not in link):
			if "#" in link :
				...
				if child_link not in child_link_list and (re.search(keyword, a_tag.text, re.IGNORECASE) or re.search(keyword, url, re.IGNORECASE)):
					child_link_list.append(child_link)
			else:
				child_link = base_url + link
				if child_link not in child_link_list and (re.search(keyword, a_tag.text, re.IGNORECASE) or re.search(keyword, link, re.IGNORECASE)):
					child_link_list.append(child_link)
...
print("all links added to file: visited_link_file_q3.txt in this folder")
